...
DR 연산 이전에 [],error가 나오는 경우를 미리 제거해서 실제 연산수를 축소

### 16%에서 계속 오류가 난 이유

- DR 연산 후, `[]`,`error`가 결과값인 경우가 정확하지 않았기 때문
  
  - `[]`: StringBuilder에서 `deleteCharAt()`으로 마지막 `,`를 제거할 때 요소가 0개인 경우도 제거되어 `]`만 출력됨
  - `error`: start >= end로 설정시, start == end일 경우는 요소가 1개 존재할 때이므로 error상태가 아님
  - 연산완료 후 start >= end인 경우는 요소가 0개,1개인 경우를 모두 포함하고 있음

### 제출 일자

2025년 1월 3일 13:14:51
