...
해당 탑의 높이와 번호를 함께 저장하는 스택을 만든 후, 

`(이전 탑 높이) < (현재 탑의 높이)`라면 **이전 탑**을 스택에서 **삭제**

→ 이후 탑에서 보낸 레이저를 해당 탑이 받게 될 경우가 없음

ex) 이전 탑 = 5, 현재 탑 = 7일때 이후의 탑에서 레이저를 쏘더라도 현재 탑(높이 7)이 받지 이전 탑(높이 5)가 받는 일은 없음

### 제출 일자

2024년 10월 20일 19:50:12
