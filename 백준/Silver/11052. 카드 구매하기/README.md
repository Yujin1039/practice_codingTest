...
전체 카드수가 n이고 index=j인 지점에서 최댓값을 찾을 때

index가 `n/2`인 곳까지 순회해도 된다.

▶ `n/2` 이후의 index에는 이전 카드값 번호들의 최댓값 모음과 겹치기 때문

ex) n=4이고 index=3일 때, card[3]+max[1]의 경우 card[1]+max[3]에서 찾은 값과 완전히 겹치는 값이 존재한다.

### 제출 일자

2025년 1월 14일 22:03:00
