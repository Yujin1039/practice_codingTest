...
큐(LinkedList)의 순회 횟수를 우측, 좌측 중 최소 값을 미리 정해두면 시간이 단축될거라 생각은 했는데 방법은 못 찾았다.

`queue.indexOf(n) <= queue.size() / 2`를 기준으로 우/좌측 중 최소값을 확정할 수 있었다.

### 제출 일자

2024년 10월 19일 20:33:07
