...
값이 `숫자`인 `char`타입 변수에 대해 `-48`을 하지않아 기존 아스키코드의 값이 그대로 반영되었기 때문

이를 반영하지 않으면

예를 들어, 알파벳으로 구성된 문자열의 각 자리 숫자 합 = 0, 0만 존재하는 문자열 각 자리 숫자 합 = 48이므로 마지막 return 문이 실행되지 않음

### 제출 일자

2024년 11월 24일 20:47:59
