...
→ 가장 큰/작은 값에만 접근할 수 있도록 만든 자료구조로, 값 삽입시 정해진 우선순위에 따라 정렬됨 

### 우선순위 큐 구현(자바)

- PriorityQueue 클래스로 구현

	`PriorityQueue<> 큐이름 = new PriorityQueue<>();`

- 정렬 순서 지정: 생성자의 매개변수로 작성

	`new PriorityQueue<>(`**comparator 작성**`);`

### 제출 일자

2024년 12월 24일 22:30:34
