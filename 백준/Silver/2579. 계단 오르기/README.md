...
k번째 계단에 대해 배열 S에는 계단에 해당하는 값이 담겨있고

2계단 연속으로 오른 경우 2차원 배열의 index = 2, 1계단만에 도달한 경우 2차원 배열의 index = 1이라고 가정

**점화식**
```java
D[K][1] = D[k-1][2] + S[k];
D[k][2] = Math.max(D[k-2][1],D[k-2][2]) + S[K];
```
...
D[i] = i번째 계단까지 올라섰을 때 밟지 않을 계단의 합의 최솟값 (i번째 계단은 밟지 않은 계단으로 가정)

**점화식**

```java
D[k] = Math.min(D[k-2],D[k-3]) + S[K] (k <= n-1)
```

실제 구할 값 = `∑ S -  Math.min(D[n-1],D[n-2])`

### 제출 일자

2024년 11월 11일 21:52:43
