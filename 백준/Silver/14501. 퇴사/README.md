...
(N+1)일차를 기준으로 수당 최댓값이 0이라고 가정하고 (상담이 없으므로) N, N-1, ... 1일차의 수당 최댓값을 구한다.

점화식의 조건은
1. `i일차 상담 X` : (i+1)일차의 수당 max
2. `i일차 상담 O` : i일차의 수당 + (i + i일의 근무일수)일차의 수당 max
   
   └ i일차의 근무 수행시, 예상 상담 소요 일수만큼 다른 일차의 상담 진행이 불가하므로

1일차의 수당 max가 (N+1)일차의 수당 최댓값

### 제출 일자

2024년 11월 26일 00:10:20
